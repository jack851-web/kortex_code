...
| 按钮 | 功能 | 使用时机 | 说明 |
|------|------|----------|------|
| **开始收集** | 启动数据收集系统 | 系统初始化完成后 | 开始收集后，按钮变灰，其他按钮激活 |
| **停止收集** | 停止数据收集 | 任何时候需要结束采集 | 如果当前有任务进行中，会保存该 episode 后停止 |
| **执行下一个任务** | 准备并执行下一个任务 | 点击"开始收集"后 | 加载任务场景、启动 Episode 录制，等待手动操作 |
| **抓取任务完毕** | 手动确认任务完成 | 完成抓放操作后 | 停止录制、判定成功、保存 Episode 数据 |
| **重做任务** | 丢弃当前数据并重新执行 | 任务执行中不满意时 | 仅任务执行中可用，丢弃当前数据后重新执行（注意不要在第一段自动过程中重做） |
| **跳过当前** | 跳过当前任务（不保存） | 当前任务无法完成时 | 丢弃当前 Episode 数据，进入下一个任务 |
| **暂停/继续** | 暂停或继续数据收集 | 需要临时中断时 | 暂停后可点击继续恢复 |
...
2. 点击「开始收集」
   → 系统进入等待状态，状态显示"等待执行任务"

3. 点击「执行下一个任务」
   → 加载任务场景，启动 Episode 录制
   → 自动 IK 到预抓取位
   → 状态显示"等待手动完成"
...
| 情况 | 操作 | 效果 |
|------|------|------|
| 任务执行中不满意 | 点击「重做任务」 | 丢弃当前数据，重新执行当前任务 |
| 场景物体位置异常，无法完成 | 点击「跳过当前」 | 丢弃当前数据，进入下一个任务 |
| 需要临时中断 | 点击「暂停」 | 暂停数据收集，可稍后「继续」 |
| 紧急停止 | 点击「停止收集」 | 保存当前进度后停止 |
