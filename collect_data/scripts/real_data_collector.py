...
def create_video_from_images(image_list: List[bytes], output_path: Path, fps: int = 30, width: int = 640, height: int = 480):
    ...
    try:
        ...
        if not writer.isOpened():
            print(f"[RealVideo] Failed to open video writer for {output_path}")
            return False
        
        for img_bytes in image_list:
            img_array = np.frombuffer(img_bytes, dtype=np.uint8)
            img_array = img_array.reshape(height, width, 3)
            # OpenCV 相机已经是 BGR 格式，直接使用
            writer.write(img_array)
        
        writer.release()
        print(f"[RealVideo] Saved: {output_path} ({len(image_list)} frames)")
        return True
    except Exception as e:
        print(f"[RealVideo] Error: {e}")
        return False
...
class RealDataCollector:
    ...
    def start_collection(self):
        ...
        print("[RealDataCollector] Started")
            ...
            try:
                ...
                with self._data_lock:
                    ...
                    state = {
                        ...
                        "cartesian_pose": cartesian.tolist(),
                    ...
                    self._current_data["actions"].append(action)
    ...
    def start_episode(self, episode_id: int, camera_names: list, task_info: Dict[str, Any]):
        ...
        print(f"[RealDataCollector] Episode {episode_id} started")
    ...
    def start_recording(self):
        ...
        print(f"[RealDataCollector] Recording started, save point: states={self._task_save_point['states']}, images={self._task_save_point['images']}")
    ...
    def discard_current_task(self):
        ...
        with self._data_lock:
            ...
            print(f"[RealDataCollector] Current task data discarded, reverted to states={self._task_save_point['states']}, images={self._task_save_point['images']}")
    ...
    def _save_episode(self, episode_id: int) -> bool:
        try:
            ...
            print(f"[RealDataCollector] Saving episode {episode_id} ({num_frames} frames)")
            
            if num_frames == 0:
                print("[RealDataCollector] Warning: No frames!")
                return False
            ...
            for key, value in self._current_data["observations"].items():
                ...
                for i, img_bytes in enumerate(value):
                    ...
                    # OpenCV 相机已经是 BGR 格式，直接保存
                    cv2.imwrite(str(img_dir / f"frame_{i:06d}.png"), img_array)
            ...
            with open(episode_dir / "action" / "action.json", "w") as f:
            ...
            self._save_progress()
            print(f"[RealDataCollector] Episode {episode_id} saved")
            return True
